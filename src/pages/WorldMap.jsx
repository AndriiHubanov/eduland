          ...
          {Array.from({ length: GRID_SIZE * GRID_SIZE }, (_, idx) => {
            ...
            return (
              <MapCell
                ...
                heroClass={cls}
                cellSize={cellSize}
                onClick={() => cellPlayer && setSelected(cellPlayer)}
              />
            )
          })}
        </div>
...
// ─── Клітинка карти ──────────────────────────────────────────
function MapCell({ cellPlayer, isOwn, heroClass, cellSize, onClick }) {
  const showName = cellSize >= 44 // показуємо ім'я тільки якщо клітинка досить велика
  ...
  return (
    <div
      ...
      className={`
        ...
        ${cellPlayer ? 'cursor-pointer active:scale-95' : 'cursor-default'}
        ${isOwn
      ...
      style={{
        width:     cellSize,
        height:    cellSize,
        boxShadow: isOwn ? 'inset 0 0 0 2px var(--accent)' : undefined,
      }}
    >
      {cellPlayer ? (
        <>
          ...
          {/* Ім'я гравця (тільки якщо клітинка велика) */}
          {showName && (
            <span
          ...
          {/* Пульсуюча крапка для власного міста */}
          {isOwn && (
        ...
        <span
          ...
          style={{ fontSize: Math.max(cellSize * 0.2, 8) }}
        >
          ·
        </span>
      )}
