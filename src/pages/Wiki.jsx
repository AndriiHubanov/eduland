...
const CHRONICLE_EVENTS = [
  { year: '2035', event: 'Перші ознаки', detail: 'Глобальні кібератаки паралізують фінансові системи. Перші попередження ігноруються.' },
  { year: '2039', event: 'Великий Колапс', detail: 'Електромагнітний каскад E9 знищив 87% інфраструктури. Уряди впали за 72 години.', highlight: true },
  { year: '2041', event: 'Епоха бункерів', detail: 'Вцілілі групи формують ізольовані общини. Перші Стражі з\'являються для захисту периметрів.' },
  { year: '2043', event: 'Пошук знань', detail: 'Детективи та Архіваріуси рейдують руїни — збирають залишки цифрових сховищ і карти.' },
  { year: '2044', event: 'Nova Academy', detail: 'Академія заснована на уламках університету. Координатори встановлюють перші торгові маршрути.' },
  { year: '2047', event: 'Відродження', detail: 'Перші міста піднімаються. Ти — один з тих, хто повертає цивілізацію.', highlight: true },
]

const HERO_ORIGINS = [
  { id: 'guardian',    icon: '🛡️', name: 'Страж',        origin: 'Колишні військові та поліцейські. Першими вийшли з бункерів, щоб захистити тих, хто вижив. Їхні фортеці — останній рубіж між хаосом і порядком.' },
  { id: 'archivist',   icon: '📚', name: 'Архіваріус',   origin: 'Науковці та програмісти, що рятували знання у дні Колапсу. Кожен їхній сервер — це частина пам\'яті, яку людство ледь не втратило назавжди.' },
  { id: 'detective',   icon: '🔍', name: 'Детектив',     origin: 'Аналітики та розвідники, які першими зрозуміли природу Колапсу. Кажуть, серед детективів є той, хто знає справжню причину катастрофи 2039.' },
  { id: 'coordinator', icon: '📡', name: 'Координатор', origin: 'Дипломати та логісти нового світу. Там, де інші бачать руїни — вони бачать торгові маршрути. Без координаторів Nova Academy не вижила б і місяця.' },
]

// ─── Sections ────────────────────────────────────────────────

const SECTIONS = [
  { id: 'lore',        label: '📜 Хроніка' },
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #0a0a0f 0%, #0c0c18 100%)' }}>
      ...
      <div className="max-w-3xl mx-auto px-4 pb-24 space-y-12 pt-6">
        ...
        {/* ════════ ХРОНІКА ════════ */}
        <section id="wiki-lore">
          <SectionHeader id="lore" label="Хроніка Колапсу" icon="📜" onView={setActiveSection} />
          <p className="text-xs text-[#555] mb-4 font-mono italic">
            // Архівні дані Nova Academy. Рівень допуску: Відкритий
          </p>

          <h3 className="wiki-h3">Хронологія подій</h3>
          <div className="relative flex flex-col mb-6">
            <div className="absolute left-[3.75rem] top-0 bottom-0 w-px bg-[var(--border)]" />
            {CHRONICLE_EVENTS.map(e => (
              <div key={e.year} className="flex gap-3 py-3 relative">
                <div className={`w-14 shrink-0 text-right font-mono text-[11px] pt-0.5 ${
                  e.highlight ? 'text-[var(--accent)]' : 'text-[#555]'
                }`}>
                  {e.year}
                </div>
                <div
                  className="w-2.5 h-2.5 rounded-full mt-0.5 shrink-0 z-10 border-2"
                  style={{
                    background: e.highlight ? 'var(--accent)' : 'var(--bg)',
                    borderColor: e.highlight ? 'var(--accent)' : 'var(--border)',
                  }}
                />
                <div className="flex-1">
                  <div className={`text-sm font-semibold ${e.highlight ? 'text-[var(--accent)]' : 'text-white'}`}>
                    {e.event}
                  </div>
                  <div className="text-xs text-[#555] mt-0.5 leading-relaxed">{e.detail}</div>
                </div>
              </div>
            ))}
          </div>

          <h3 className="wiki-h3">Класи виживших</h3>
          <div className="grid grid-cols-1 gap-3 mb-4">
            {HERO_ORIGINS.map(h => (
              <div key={h.id} className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-xl">{h.icon}</span>
                  <span className="font-bebas text-base tracking-wider text-white">{h.name}</span>
                </div>
                <p className="text-xs text-[#555] leading-relaxed italic">{h.origin}</p>
              </div>
            ))}
          </div>
          <Note>Кожен гравець обирає клас на початку сезону. Клас впливає на назви замку, бонуси будівель і класові місії.</Note>
        </section>
